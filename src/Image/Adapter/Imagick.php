...
namespace Azbe\Image\Adapter;

use Azbe\Image\Image;
use Azbe\Image\Exception;
...
class Imagick extends Image
{
  ...
  public function __construct($options = array())
  {
    ...
    if( !class_exists('Imagick', false) ) {
      throw new Exception('Imagick library is not installed');
    }
   ...
   * @throws Azbe\Image\Exception If unable to open
   */
  public function open($file)
  {
    ...
    if( !$return || !$this->_checkOpenImage(false) ) {
      $this->_resource = null;
      throw new Exception(sprintf('Unable to open image "%s"', $file));
    }
    return $this;
  }
   ...
   * @return Engine_Image_Adapter_Gd
   * @throws Azbe\Image\Exception If unable to write
   */
  public function write($file = null, $type = 'jpeg')
  {
    ...
    if( !$return ) {
      ...
      throw new Exception(sprintf('Unable to write image to file "%s"', $file));
    }
    return $this;
  }
   ...
   * @return mixed
   * @throws Azbe\Image\Exception If unable to output
   */
   ...
   * @return Engine_Image_Adapter_Gd
   * @throws Azbe\Image\Exception If unable to resize
   */
   ...
   * @param integer $h
   * @return Engine_Image_Adapter_Gd
   * @throws Azbe\Image\Exception If unable to crop
   */
  public function crop($x, $y, $w, $h)
  {
    ...
    try {
      ...
      throw new Exception(sprintf('Unable to crop image: %s',
          $e->getMessage()), $e->getCode());
    }
  ...
  public function resample($srcX, $srcY, $srcW, $srcH, $dstW, $dstH)
  {
    ...
    try {
      ...
      throw new Exception(sprintf('Unable to resample image: %s',
          $e->getMessage()), $e->getCode());
    }
  ...
  public function rotate($angle)
  {
    ...
    try {
      ...
      throw new Exception(sprintf('Unable to rotate image: %s',
          $e->getMessage()), $e->getCode());
    }
  ...
  public function flip($horizontal = true)
  {
    ...
    try {
      ...
      throw new Exception(sprintf('Unable to flip image: %s',
          $e->getMessage()), $e->getCode());
    }
  ...
  protected function _checkOpenImage($throw = true)
  {
    if( !($this->_resource instanceof Imagick) ) {
      if( $throw ) {
        throw new Exception('No open image to operate on.');
      } else {
        return false;
      }
    } else {
